...
void init_browsing(int sfs_handle)
{
	...
	used_blocks = (byte1_t *)(calloc(sb.partition_size, sizeof(byte1_t)));
	if (!used_blocks)
	{
...
static int get_data_block(int sfs_handle)
{
	int i;
...
static void put_data_block(int sfs_handle, int i)
{
	used_blocks[i] = 0;
}
...
void sfs_create(int sfs_handle, char *fn)
{
	...
	strncpy(fe.name, fn, FS_FILENAME_LEN);
	fe.name[FS_FILENAME_LEN] = 0;
	fe.size = 0;
	fe.timestamp = time(NULL);
	fe.perms = 07;
	for (i = 0; i < FS_DATA_BLOCK_CNT; i++)
	{
		fe.blocks[i] = 0;
	}
...
void sfs_remove(int sfs_handle, char *fn)
{
	...
	for (block_i = 0; block_i < FS_DATA_BLOCK_CNT; block_i++)
	{
		if (!fe.blocks[block_i])
		{
			break;
		}
		put_data_block(sfs_handle, fe.blocks[block_i]);
	}
...
void browse_sfs(int sfs_handle)
{
	...
	printf("Welcome to SFS Browsing Shell\n\n");
